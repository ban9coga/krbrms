...
export default function CertificateClient({ eventId }: { eventId: string }) {
  ...
  const download = async (rider: CertificateRider) => {
    setError('')
    ...
    try {
      ...
      anchor.href = url
      anchor.download = `e-sertifikat-${rider.name.replace(/[^a-z0-9]+/gi, '-').toLowerCase()}.pdf`
      ...
      <main className="mx-auto grid w-full max-w-[900px] gap-5 px-4 pb-20 pt-6 sm:px-6 md:pt-10">
        <section className="rounded-[2rem] border border-[#4f372b] bg-[#1d0d07] px-5 py-7 text-[#fff8e8] shadow-[0_28px_70px_rgba(55,23,9,0.24)] sm:px-8">
          ...
          <p className="mt-3 max-w-2xl text-sm font-semibold leading-6 text-[#c9b7a5] md:text-base">
            Masukkan nomor registrasi dan nomor WhatsApp wali yang digunakan saat mendaftar. Setelah cocok, pilih rider untuk mengunduh PDF sertifikat.
          </p>
        </section>
          ...
          <div className="grid gap-4 md:grid-cols-2">
            <label className="grid gap-2 text-xs font-black uppercase text-[#5f4638]">
              ...
              <input
                ...
                onChange={(event) => setRegistrationCode(event.target.value.toUpperCase().slice(0, 24))}
              ...
              {result.riders.map((rider) => (
                <article key={rider.id} className="flex flex-col gap-3 rounded-2xl border border-[#d9c9ae] bg-white p-4 sm:flex-row sm:items-center sm:justify-between">
                  ...
                  <button
                    ...
                    onClick={() => void download(rider)}
                    className="inline-flex min-h-11 items-center justify-center rounded-full bg-[#1d0d07] px-5 py-3 text-xs font-black uppercase text-[#fff8e8] hover:bg-[#3e2113] disabled:cursor-wait disabled:opacity-60"
                  >
                    {downloadingId === rider.id ? 'Membuat PDF...' : 'Unduh PDF'}
                  </button>
                </article>
              ))}
            </div>
