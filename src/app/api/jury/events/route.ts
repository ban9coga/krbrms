...
export async function GET(req: Request) {
  const auth = await requireJury(req, ['CHECKER', 'FINISHER', 'RACE_DIRECTOR', 'ADMIN', 'super_admin'])
  if (!auth.ok) return NextResponse.json({ error: auth.error }, { status: auth.status })

  const { searchParams } = new URL(req.url)
  const statuses = searchParams.get('status')?.split(',') ?? ['LIVE', 'UPCOMING']

  const accessibleEventIds = await getAccessibleEventIds(auth.user.id, ['CHECKER', 'FINISHER', 'RACE_DIRECTOR', 'ADMIN', 'super_admin'])
