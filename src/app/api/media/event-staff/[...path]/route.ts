...
export async function GET(_req: Request, { params }: { params: Promise<{ path: string[] }> }) {
  ...
  if (!isSafePath(storagePath)) return NextResponse.json({ error: 'Invalid media path' }, { status: 400 })

  const { data, error } = await adminClient.storage.from(BUCKET).download(storagePath)
  if (error || !data) return NextResponse.json({ error: error?.message || 'Media not found' }, { status: 404 })
